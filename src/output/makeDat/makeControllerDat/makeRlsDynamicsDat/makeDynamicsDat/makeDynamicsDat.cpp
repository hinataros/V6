...
void RLS::Output::makeDynamicsDat(Config &config, Info &info)
{
  ...
  minipage = baseTransDes(config, info, load);
  if(config.graph.tex){
  ...
  name = "101_baseRotDes";
  minipage = baseRotDes(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  name = "110_eeTransDes";
  minipage = eeTransDes(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  name = "111_eeRotDes";
  minipage = eeRotDes(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  minipage = baseTransErr(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  name = "121_baseRotErr";
  minipage = baseRotErr(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  name = "122_comErr";
  minipage = comErr(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
		  "CoM error values."
		  );
  ...
  name = "130_rcMomRef";
  minipage = rcMomRef(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  minipage = eeWrenchRef(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  minipage = formulatedMom(config, info, load);
  if(config.graph.tex){
    makeFigureTex(config, name, minipage,
  ...
  ofstream rlsDynamicsGp(pathGp.c_str());
  if(!rlsDynamicsGp)
  ...
  ofstream tex(pathTex.c_str(), ios::app);
  if(!tex)
    cout << path << ": " << endl << "file open error..." << endl;
  else{
    tex << input << endl;
