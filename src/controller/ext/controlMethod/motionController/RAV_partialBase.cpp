...
VectorXd RLS::Ext::RAV_partialBase(RlsDynamics *io)
{
  if(debug) DEBUG;
  ...
  // VectorXd dthnRef = NN*N(JwBar)*jointLimitStop(io, "vel");
  // // VectorXd dthnRef = NN*N(JwBar)*(jointLimitAvoidance(io, "vel") + jointLimitStop(io, "vel"));
