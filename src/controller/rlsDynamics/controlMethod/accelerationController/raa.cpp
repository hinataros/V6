...
VectorXd RLS::RlsDynamics::raa(const TreeModel::Info &info)
{
  if(debug) DEBUG;
  ...
  VectorXd ddthwRef = pInv(JwBar)*(dwTildeRef - (dJw + KDlC*Jw)*dth) + dpInv(JwBar, dJwBar)*(wTilde);

  ddthRef = ddthcRef + ddthmRef + ddthwRef + NN*N(JwBar)*ddthD(info);

  // o(Jw*dth);
  // o(Jw*ddthRef + dJw*dth);
  // gc;
